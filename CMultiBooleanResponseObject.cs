    ...
    class CMultiBooleanResponseObject : CResponseObject
    {
        ...
        public override ISurveyItemResponse Response
        {
            ...
            set
            {
                ...
                if (value.IsAnswered)
                {
                    AnswerStates.SetValue(EAnswerState.Selected, value.Value.Select((c, ndx) => (c == '1') ? ndx : -1).Where(val => val != -1).ToArray());
                    AnswerStates.SetValue(EAnswerState.Unselected, value.Value.Select((c, ndx) => (c == '0') ? ndx : -1).Where(val => val != -1).ToArray());
                }
                else
