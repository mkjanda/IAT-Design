...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema"
	...
	<xsl:template name="GenerateProcessItemFunctions">
		<xsl:for-each select="for $i in 1 to count(//EventList/child::BeginIATBlock) return //EventList/child::BeginIATBlock[$i]">
			<xsl:variable name="i" select="position()" />
			<xsl:element name="ProcessItemsFunctions">
				<xsl:attribute name="BlockNum" select="$i" />
				<xsl:variable name="blockStart" select="//EventList/child::BeginIATBlock[$i]" />
				<xsl:variable name="items" select="$blockStart/following-sibling::IATItem[position() le xs:integer($blockStart/NumItems)]" />
