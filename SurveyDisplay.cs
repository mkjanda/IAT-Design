...
namespace IATClient
{
    partial class SurveyDisplay : UserControl
    {
        ...
        /// </summary>
        static public Padding InteriorPadding = new Padding(5, 5, 5, 20);

        /// <summary>
        /// the width of the line that pads the left and right of currently selected questions
        /// </summary>
        private const int SelectionLineWidth = 3;
            ...
            set
            {
                ...
                for (int ctr = 0; ctr < _Survey.Items.Count; ctr++)
                {
                    ...
                    else if (_Survey.Items[ctr].ItemType == SurveyItemType.SurveyImage)
                    {
                        ...
                        Controls.Add(sid);
                        sid.OnHeightChanged = new Action(RecalcSize);
        ...
        private void SurveyDisplay_Paint(object sender, PaintEventArgs e)
        {
            for (int ctr = 0; ctr < QuestionEdits.Count; ctr++)
            {
                ...
                {
                    e.Graphics.FillRectangle(BlueHorizon, new Rectangle(0, QuestionEdits[ctr].Location.Y - QuestionDisplayMargin.Top, SelectionLineWidth,
                        QuestionEdits[ctr].Size.Height + QuestionDisplayMargin.Vertical));
                    
                    e.Graphics.FillRectangle(BlueHorizon, new Rectangle(QuestionEdits[ctr].Location.X + QuestionEdits[ctr].Width + QuestionDisplayMargin.Right - SelectionLineWidth, 
                        QuestionEdits[ctr].Location.Y - QuestionDisplayMargin.Top, SelectionLineWidth, QuestionEdits[ctr].Size.Height + QuestionDisplayMargin.Vertical));

                    e.Graphics.FillRectangle(Rosebloom, new Rectangle(SelectionLineWidth, QuestionEdits[ctr].Location.Y - QuestionDisplayMargin.Top, SelectionLineWidth,
                        QuestionEdits[ctr].Size.Height + QuestionDisplayMargin.Vertical));

                    e.Graphics.FillRectangle(Rosebloom, new Rectangle(QuestionEdits[ctr].Location.X + QuestionEdits[ctr].Width + QuestionDisplayMargin.Right - (SelectionLineWidth << 1), 
                        QuestionEdits[ctr].Location.Y - QuestionDisplayMargin.Top, SelectionLineWidth, QuestionEdits[ctr].Size.Height + QuestionDisplayMargin.Vertical));
                ...
                else if (QuestionEdits[ctr].Selected || QuestionEdits[ctr].IsUnique)
                {
                    ...
                    e.Graphics.FillRectangle(br, new Rectangle(0, QuestionEdits[ctr].Location.Y - QuestionDisplayMargin.Top, InteriorPadding.Left,
                        QuestionEdits[ctr].Size.Height + QuestionDisplayMargin.Vertical));
                    
                    e.Graphics.FillRectangle(br, new Rectangle(this.Width - InteriorPadding.Right, QuestionEdits[ctr].Location.Y - QuestionDisplayMargin.Top, 
        ...
        /// <summary>
        /// recalculates the client size of the survey display
        /// </summary>
        public void RecalcSize()
        {
            ...
            SuspendLayout();
            int ctrStart = 0;
            int oldDocHeight = DocHeight;
            List<Point> qEditLocations = new List<Point>();
            DocHeight = QuestionDisplayMargin.Top;
            if (HasCaption)
            {
                ctrStart = 1;
                QuestionEdits[0].Location = new Point(QuestionDisplayMargin.Top, 0);
                DocHeight += QuestionEdits[0].Height + QuestionDisplayMargin.Vertical;
            }
            for (int ctr = ctrStart; ctr < QuestionEdits.Count; ctr++)
            {
                QuestionEdits[ctr].Location = new Point(InteriorPadding.Left + QuestionDisplayMargin.Left, DocHeight + QuestionDisplayMargin.Top);
                DocHeight += QuestionEdits[ctr].Height + QuestionDisplayMargin.Vertical;
            }
            ResumeLayout(false);
            Invalidate();
...
/*
        private QuestionDisplay CreateAppendedQuestion()
        {
            QuestionDisplay qEdit = new QuestionDisplay(_Survey);
            Controls.Add(qEdit);
            QuestionEdits.Add(qEdit);
            qEdit.Size = new Size(this.Size.Width - Padding.Horizontal - QuestionDisplayMargin.Horizontal, 1);
            qEdit.Location = new Point(Padding.Left + QuestionDisplayMargin.Left, DocHeight - Padding.Bottom + QuestionDisplayMargin.Top);
            qEdit.BackColor = this.BackColor;
            return qEdit;
        }

        public QuestionDisplay CreateInsertedQuestion()
        {
            QuestionDisplay qEdit = new QuestionDisplay(_Survey);
            Controls.Add(qEdit);
        ...
        public void PasteInsert()
        {
            for (int ctr = 0; ctr < Clipboard.Count; ctr++)
            {
                ...
                else
                {
                    ...
                    {
                        QuestionEdits.Insert(1, qEdit);
                        Survey.Items.Insert(1, qEdit.SurveyItem);
                    }
                    else
                    {
                        QuestionEdits.Insert(LastSelectedQuestionNdx, qEdit);
