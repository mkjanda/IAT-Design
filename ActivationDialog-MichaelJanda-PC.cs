...
namespace IATClient
{
    partial class ActivationDialog : Form
    {
        ...
        private void Activate_Click(object sender, EventArgs e)
        {
            ...
            (new Action<String, String, String, String, String>(actObj.Activate)).BeginInvoke(FName.Text, LName.Text, EMail.Text, TitleBox.Text, ProductKey.Text, new AsyncCallback(ActivationComplete), actObj);
        }

        private void ActivationComplete(IAsyncResult aResult)
        {
            ActivationObject actObj = aResult.AsyncState as ActivationObject;
            ...
            } else if (actObj.ActivationResult == ActivationResponse.EResult.InvalidProductCode)
                this.Invoke(new Action(() => { ActivateMessage.Text = "Invalid Product Code"; }));
        }
        ...
        private void HaveVerifiedButton_Click(object sender, EventArgs e)
        {
            ...
            Func<EMailConfirmation.EConfirmResult> del = confirm.ConfirmEMailVerification;
            del.BeginInvoke(new AsyncCallback(EMailConfirmationCheckComplete), del);
        }

        private void EMailConfirmationCheckComplete(IAsyncResult result)
        {
            Func<EMailConfirmation.EConfirmResult> del = (Func<EMailConfirmation.EConfirmResult>)result.AsyncState;
            EMailConfirmation.EConfirmResult confirmResult = del.EndInvoke(result);
            if (confirmResult == EMailConfirmation.EConfirmResult.success)
                this.Invoke((Action<bool>)OnEMailConfirmed, true);
            else
                this.Invoke((Action<bool>)OnEMailConfirmed, false);
        }

        private void OnEMailConfirmed(bool result)
        {
            ResendVerificationButton.Enabled = true;
            HaveVerifiedButton.Enabled = true;
            if (result == true)
            ...
            else
            {
                ...
                LName.Enabled = true;
                EMail.Enabled = true;
                MessageBox.Show("We have yet to receive your email verification.  If you have not received an email from IAT Software instructing you on how to complete activation of your product, first check your Spam folder and, if no such email is present there, click \"Resend Verification EMail\"", "EMail Not Verified", MessageBoxButtons.OK);
            }
        }
        ...
        private void ResendVerificationButton_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Another verification email will be sent to the email address provided in the Activation Form.  Click OK to send this request or click cancel if you wish to return to the Activation Form to edit your email address.", "Resend Verification EMail", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                ...
                Task.Run(() => { resendConfirm.ResendEMailVerification(EMail.Text); }).Wait();
                ...
                if (resendConfirm.FinalTransaction.Transaction == TransactionRequest.ETransaction.TransactionFail)
                    ...
                    LocalStorage.Activation[LocalStorage.Field.VerificationCode] = resendConfirm.FinalTransaction.StringValues["VerificationCode"];
