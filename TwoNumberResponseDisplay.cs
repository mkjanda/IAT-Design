...
namespace IATClient
{
    abstract class TwoNumberResponseDisplay : OneNumberResponseDisplay
    {
        ...
        protected override void LayoutControl()
        {
            ...
            int val1Width = TextRenderer.MeasureText(Value1Label, ControlFont).Width + LengthEditMargin.Horizontal + Value1.Width;
            int val2Width = TextRenderer.MeasureText(Value2Label, ControlFont).Width + LengthEditMargin.Horizontal + Value2.Width;
            if (val1Width + val2Width + 2 * LengthEditMargin.Horizontal > ClientSize.Width)
            {
                Value1.Location = new Point(LengthEditMargin.Horizontal + TextRenderer.MeasureText(Value1Label, ControlFont).Width, Padding.Top + LinePadding - Value1.Margin.Top);
                Value2.Location = new Point(LengthEditMargin.Horizontal + TextRenderer.MeasureText(Value2Label, ControlFont).Width, Value1.Bottom + Padding.Top + 2 * LinePadding - Value2.Margin.Top);
                this.Height = Math.Min(Value1.Location.Y + Value1.Size.Height, Value2.Location.Y + Value2.Size.Height);
            } else
            {
                Value1.Location = new Point(LengthEditMargin.Horizontal + TextRenderer.MeasureText(Value1Label, ControlFont).Width, Padding.Top + LinePadding - Value1.Margin.Top);
                Value2.Location = new Point((ClientSize.Width >> 1) + LengthEditMargin.Horizontal + TextRenderer.MeasureText(Value2Label, ControlFont).Width, Padding.Top + LinePadding - Value2.Margin.Top);
                this.Height = Math.Min(Value1.Location.Y + Value1.Size.Height, Value2.Location.Y + Value2.Size.Height);
            }
        ...
        protected override void ResponseDisplay_Paint(object sender, PaintEventArgs e)
        {
            ...
            e.Graphics.DrawString(Value1Label, ControlFont, br, new Point(LengthEditMargin.Left, Padding.Top + LinePadding));
            if (!Value1.Visible)
            ...
            if (Value1.Location.Y == Value2.Location.Y) {
                e.Graphics.DrawString(Value2Label, ControlFont, br, new Point((ClientSize.Width >> 1) + LengthEditMargin.Left, Padding.Top + LinePadding));
                if (!Value2.Visible)
                    e.Graphics.DrawString(Value2.Value.ToString(), DisplayFont, blackBr, new Point(Value2.Left, Padding.Top + LinePadding));
            }
            else {
                e.Graphics.DrawString(Value2Label, ControlFont, br, new Point(LengthEditMargin.Left, Padding.Top + Value1.Height + (LinePadding << 1)));
                if (!Value2.Visible)
