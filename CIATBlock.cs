...
namespace IATClient
{
    public class CIATBlock : IContentsItem, IValidatedItem, IDisposable, IPackagePart
    {
        protected CIAT IAT;
        private readonly DILambdaGenerated BlockPreviewLambda;
        private Uri PreviewUri;
        private int _NumPresentations;
        protected AlternationGroup AltGroup = null;
        private List<Tuple<String, Uri>> ItemTuples = new List<Tuple<String, Uri>>();
        ...
        public Type BaseType { get { return typeof(CIATBlock); } }
        public static String _MimeType{ get { return "text/xml+" + typeof(CIATBlock).ToString(); } }
        public String MimeType { get { return "text/xml+" + typeof(CIATBlock).ToString(); } }
        ...
        public CIATKey Key
        {
            get
            {
                try
                {
                    return CIAT.SaveFile.GetRelationshipsByType(this.URI, BaseType, typeof(CIATKey)).Select(pr => CIAT.SaveFile.GetIATKey(pr.TargetUri)).First();
                }
                catch (Exception)
                {
                    return null;
            ...
            set
            {
                try
                {
                    ...
                    if (oldKey != null)
                    {
                        ...
                        CIAT.ActivityLog.LogEvent(ActivityLog.EventType.Detached, oldKey.URI, URI);
                    }
                    ...
                    CIAT.ActivityLog.LogEvent(ActivityLog.EventType.Attached, value.URI, URI);
        ...
        public int NumPresentations
        {
            get
            {
                if (CIAT.SaveFile.IAT.RandomizationType == IATConfigFileNamespace.ConfigFile.ERandomizationType.SetNumberOfPresentations)
                {
        ...
        public void InsertItem(int ndx, CIATItem item)
        {
            ...
            if (IsDynamicallyKeyed)
                item.AddParentBlock(this, KeyedDirection.DynamicNone);
            else
                item.AddParentBlock(this, KeyedDirection.None);
        ...
        public void Preview(IImageDisplay previewPanel)
        {
            ...
            (CIAT.SaveFile.GetDI(PreviewUri) as DIPreview).PreviewPanel = previewPanel;
        }
