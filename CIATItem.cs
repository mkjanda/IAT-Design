    ...
    public class CIATItem : IValidatedItem, IDisposable, IPackagePart, IThumbnailPreviewable
    {
        ...
        public Uri StimulusUri
        {
            ...
            set
            {
                ...
                if (oldStim != null)
                {
                    CIAT.ActivityLog.LogEvent(ActivityLog.EventType.Detached, _StimulusUri, URI);
                ...
                if (stim != null)
                {
                    ...
                    CIAT.ActivityLog.LogEvent(ActivityLog.EventType.Attached, value, URI);
                }
        ...
        public IStimulus Stimulus
        {
            get
            {
                try
                {
                    if (StimulusUri == null)
                        return null;
                    IStimulus stim = CIAT.SaveFile.GetDI(StimulusUri) as IStimulus;
                    if (stim == null)
                    {
                        stim = new DINull();
                        _StimulusUri = stim.URI;
                    }
                    stim.ThumbnailPreviewPanel = ThumbnailPreviewPanel;
                    return stim;
                }
            ...
            set
            {
                value.ThumbnailPreviewPanel = ThumbnailPreviewPanel;
                StimulusUri = value.URI;
            }
        }
        ...
        public void SetKeyedDirection(Uri parentBlockUri, KeyedDirection keyedDir)
        {
            if (ParentBlockUris[parentBlockUri].Item1 == keyedDir)
                return;
            if ((ParentBlockUris[parentBlockUri].Item1 == KeyedDirection.Left) || (ParentBlockUris[parentBlockUri].Item1 == KeyedDirection.Right))
            {
                ...
                if ((parentBlockUri.Equals(baseBlock.URI)) || (changingBlock == 2) || (changingBlock == 3))
                    newBaseKeyedDir = keyedDir;
                else if (baseBlock.IndexInContainer == 0)
                    newBaseKeyedDir = keyedDir;
                else
                    newBaseKeyedDir = oldBaseKeyedDir;
                List<Uri> blockUris = ParentBlockUris.Keys.ToList();
                foreach (Uri u in blockUris)
                {
                    int nBlock = CIAT.SaveFile.GetIATBlock(u).IndexInContainer;
                    if ((OriginatingBlock == 0) && (nBlock > 3) && (changingBlock < 3))
                        continue;
                    if ((OriginatingBlock == 0) && (nBlock < 3) && (changingBlock > 3))
                        continue;
                    if (ParentBlockUris[u].Item1 == KeyedDirection.Left)
                    {
                        (CIAT.SaveFile.GetDI(ParentBlockUris[u].Item2) as DIPreview).RemoveComponent(LayoutItem.LeftResponseKeyOutline, false);
                        (CIAT.SaveFile.GetDI(ParentBlockUris[u].Item2) as DIPreview).AddComponent(CIATLayout.IRightKeyValueOutlineUri, LayoutItem.RightResponseKeyOutline);
                    }
                    else
                    {
                        (CIAT.SaveFile.GetDI(ParentBlockUris[u].Item2) as DIPreview).RemoveComponent(LayoutItem.RightResponseKeyOutline, false);
                        (CIAT.SaveFile.GetDI(ParentBlockUris[u].Item2) as DIPreview).AddComponent(CIATLayout.ILeftKeyValueOutlineUri, LayoutItem.LeftResponseKeyOutline);
                    }
            ...
            else if ((ParentBlockUris.Keys.Count == 1) && ((ParentBlockUris.First().Value.Item1 == KeyedDirection.None) || (ParentBlockUris.First().Value.Item1 == KeyedDirection.DynamicNone)))
            {
                if (ParentBlockUris.First().Value.Item1 == KeyedDirection.None) {
                    if (keyedDir == KeyedDirection.Left)
                        (CIAT.SaveFile.GetDI(ParentBlockUris.First().Value.Item2) as DIPreview).AddComponent(CIATLayout.ILeftKeyValueOutlineUri, LayoutItem.LeftResponseKeyOutline);
                    if (keyedDir == KeyedDirection.Right)
                        (CIAT.SaveFile.GetDI(ParentBlockUris.First().Value.Item2) as DIPreview).AddComponent(CIATLayout.IRightKeyValueOutlineUri, LayoutItem.RightResponseKeyOutline);
                }
                ParentBlockUris[ParentBlockUris.First().Key] = new Tuple<KeyedDirection, Uri>(keyedDir, ParentBlockUris.First().Value.Item2);
            } else
                throw new ArgumentException("Illegal attempt to set IAT item keyed direction");
        ...
        public void ValidateItem(Dictionary<IValidatedItem, CValidationException> ErrorDictionary)
        {
            ...
            CLocationDescriptor loc = new CIATBlockLocationDescriptor(parentBlock, this);
            if (Stimulus.Type == DIType.Null) {
