            ...
            set
            {
                ...
                SurveyView.Resize += (sender, args) =>
                {
                    if (Interlocked.Equals(SurveyViewRefreshStatus, 3) || Interlocked.Equals(SurveyViewRefreshStatus, 2))
                        return;
                    if (Interlocked.CompareExchange(ref SurveyViewRefreshStatus, 2, 1) == 1)
                        return;
                    if (Interlocked.CompareExchange(ref SurveyViewRefreshStatus, 1, 0) != 0)
                        return;
                    Action OnResize = () =>
                    {
