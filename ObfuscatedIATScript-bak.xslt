...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema"
  ...
  <xsl:variable name="Classes">
    ...
    <xsl:element name="Class">
      <xsl:attribute name="ClassName" select="'EventInitializer'" />
      <xsl:element name="Super">
        <xsl:attribute name="Has" select="'no'" />
      </xsl:element>
      <xsl:element name="Constructor">
        ...
        <xsl:variable name="constructorBodyElems">
          <xsl:text>
          this.InstructionBlocks = new Array();
          this.NumItemsAry = new Array();
          this.piFunctions = new Array();
          this.sourceAry = 1;
          this.IATBlocks = new Array();
        </xsl:text>
        </xsl:variable>
        <xsl:element name="ConstructorBody">
      ...
      <xsl:element name="PrototypeChain">
        <xsl:for-each select="//IATEventList/IATEvent[@EventType eq 'BeginIATBlock']">
          ...
          <xsl:element name="Function">
            <xsl:attribute name="FunctionName" select="concat('BuildPIFArray', position())" />
            <xsl:element name="Params" />
            <xsl:variable name="functionBodyElems">
              <xsl:if test="//RandomizationType eq 'SetNumberOfPresentations'">
                <xsl:value-of select="concat('this.NumItemsAry.push(', NumPresentations, ');&#x0A;')" />
              </xsl:if>
              <xsl:if test="//RandomizationType ne 'SetNumberOfPresentations'">
                <xsl:value-of select="concat('NumItemsAry.push(', $numItems, ');&#x0A;')" />
              </xsl:if>
              <xsl:text>
              var pifAry = new Array();
              this.piFunctions.push(pifAry);
            </xsl:text>
          ...
          <xsl:element name="Function">
            <xsl:attribute name="FunctionName" select="concat('BuildBlock', position())" />
            <xsl:element name="Params" />
            <xsl:variable name="functionBodyElems">
              <xsl:choose>
                <xsl:when test="(//Is7Block eq 'True') and (//RandomizationType eq 'SetNumberOfPresentations')">
                  <xsl:value-of select="concat('var iatBlock = new IATBlock(', BlockNum, ', ', $blockPosition, ', ', NumPresentations, ', ', AlternatedWith, ');&#x0A;')"/>
                </xsl:when>
                <xsl:when test="(//Is7Block eq 'False') and (//RandomizationType eq 'SetNumberOfPresentations')" >
                  <xsl:value-of select="concat('var iatBlock = new IATBlock(', BlockNum, ', ', $blockPosition, ', ', NumPresentations, ', ', AlternatedWith, ');&#x0A;')" />
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="concat('var iatBlock = new IATBlock(', BlockNum, ', ', $blockPosition, ', ',  NumItems, ', ', AlternatedWith, ');&#x0A;')" />
                </xsl:otherwise>
              </xsl:choose>
              ...
              <xsl:value-of select="'this.IATBlocks.push(iatBlock);&#x0A;'" />
            </xsl:variable>
            <xsl:element name="FunctionBody">
              <xsl:for-each select="tokenize($functionBodyElems, '&#x0A;')">
                <xsl:if test="string-length(normalize-space(.)) gt 0">
        ...
        <xsl:element name="Function">
          <xsl:attribute name="FunctionName" select="'PopulateItemBlocks'" />
          <xsl:element name="Params" />
          <xsl:variable name="functionBodyElems">
            <xsl:text>
            var sourceAry = 1, ctr, ctr2, lesserAry, greaterAry, iatItem;
          </xsl:text>
            <xsl:value-of select="concat('for (ctr = 0; ctr &lt; ', count(//IATEventList/IATEvent[@EventType eq 'BeginIATBlock']), '; ctr++) {&#x0A;')" />
            <xsl:choose>
              <xsl:when test="(//Is7Block eq 'True') and (//RandomizationType eq 'SetNumberOfPresentations')">
                <xsl:text>
                var Items1 = new Array(), Items2 = new Array();
                sourceAry = ((sourceAry == 2) || (ctr == 0)) ? 1 : 2;
              </xsl:text>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="'var Items = new Array();&#x0A;'" />
              </xsl:otherwise>
            </xsl:choose>
            <xsl:text>
    		    for (ctr2 = 0; ctr2 &lt; piFunctions[ctr].length; ctr2++)
    	        this.piFunctions[ctr][ctr2].call();
            ...
            for (ctr2 = 0; ctr2 &lt; this.NumItemsAry[ctr]; ctr2++) {
          </xsl:text>
            <xsl:choose>
              <xsl:when test="(//Is7Block eq 'True') and (//RandomizationType eq 'SetNumberOfPresentations')">
                <xsl:text>
                if (lesserAry.length == 0)
                  iatItem = greaterAry[Math.floor(Math.random() * greaterAry.length)];
                else {
                if (sourceAry == 1) {
              ...
              <xsl:otherwise>
                <xsl:value-of select="'iatItem = Items[Math.floor(Math.random() * Items.length)];&#x0A;'" />
              </xsl:otherwise>
            </xsl:choose>
            <xsl:text>
            this.IATBlocks[ctr].AddItem(iatItem);
            }
            this.IATBlocks[ctr].setEndBlockEvent(new IATEndBlock());
          </xsl:text>
          </xsl:variable>
          <xsl:element name="FunctionBody">
            <xsl:for-each select="tokenize($functionBodyElems, '&#x0A;')">
    ...
    <xsl:element name="Function">
      <xsl:attribute name="FunctionName" select="'PopulateInstructionBlocks'" />
      <xsl:element name="Params" />
      <xsl:variable name="functionBodyElems">
        <xsl:for-each select="//IATEventList/IATEvent[@EventType eq 'BeginInstructionBlock']">
          ...
          <xsl:value-of select="'this.InstructionBlocks.push(instructionBlock);&#x0A;'" />
        </xsl:for-each>
      </xsl:variable>
      <xsl:element name="FunctionBody">
        <xsl:for-each select="tokenize($functionBodyElems, '&#x0A;')">
    ...
    <xsl:element name="Function">
      <xsl:attribute name="FunctionName" select="'BuildEventList'" />
      <xsl:element name="Params" />
      <xsl:variable name="functionBodyElems">
        ...
        <xsl:for-each select="$EventList/Block">
          ...
          <xsl:if test="@AlternatedWith ne '-1'">
            <xsl:value-of select="'if (!bAlternate)&#x0A;'" />
            <xsl:value-of select="concat('ndx = ', $ndx, ';&#x0A;')"/>
          ...
          <xsl:if test="@BlockType eq 'IAT'">
            <xsl:text>
              EventList.push(this.IATBlocks[ndx].getBeginBlockEvent());
              for (ctr2 = 0; ctr2 &lt; IATBlocks[ndx].getNumItems(); ctr2++) {
                EventList.push(this.IATBlocks[ndx].getItem(ctr2));
              }
              EventList.push(this.IATBlocks[ndx].getEndBlockEvent());
            </xsl:text>
          </xsl:if>
          <xsl:if test="@BlockType eq 'Instruction'">
            <xsl:text>
              for (ctr2 = 0; ctr2 &lt; this.InstructionBlocks[ndx].getNumScreens(); ctr2++) {
                EventList.push(this.InstructionBlocks[ndx].getScreen(ctr2));
              }
            </xsl:text>
          </xsl:if>
        </xsl:for-each>
        <xsl:value-of select="'EventList.push(new IATSubmitEvent());&#x0A;'" />
      </xsl:variable>
      <xsl:element name="FunctionBody">
        <xsl:for-each select="tokenize($functionBodyElems, '&#x0A;')">
          <xsl:if test="string-length(normalize-space(.)) gt 0">
  ...
  <xsl:variable name="Functions">
    ...
    <xsl:element name="Function">
      ...
      <xsl:variable name="functionBodyElems">
        <xsl:element name="Code">EventCtr = 0;</xsl:element>
        <xsl:element name="Code">var generator = new EventInitializer();</xsl:element>
        <xsl:for-each select="1 to count(//IATEventList//IATEvent[@EventType eq 'BeginIATBlock'])">
          <xsl:element name="Code">
            <xsl:value-of select="concat('generator.BuildPIFArray', ., '();')" />
          </xsl:element>
          <xsl:element name="Code">
            <xsl:value-of select="concat('generator.BuildBlock', ., '();')"/>
          </xsl:element>
        </xsl:for-each>
        <xsl:element name="Code">generator.PopulateItemBlocks();</xsl:element>
        <xsl:element name="Code">generator.PopulateInstructionBlocks();</xsl:element>
        <xsl:element name="Code">generator.BuildEventList();</xsl:element>
      </xsl:variable>
      <xsl:element name="FunctionBody">
        <xsl:for-each select="$functionBodyElems/Code">
          <xsl:element name="Code">
