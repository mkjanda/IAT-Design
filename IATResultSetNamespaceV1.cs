...
namespace IATClient
{
    namespace IATResultSetNamespaceV1
    {
        ...
        public class IATResultSet : INamedXmlSerializable, IResultSet
        {
            ...
            protected double MeanTotalLatency(List<IIATItemResponse> responses)
            {
                return responses.Select(r => r.ResponseTime).Average();
            }

            protected double SDTotalLatency(List<IIATItemResponse> responses)
            {
                double mean = responses.Select(r => r.ResponseTime).Average();
                return Math.Sqrt(responses.Select(r => (double)r.ResponseTime).Aggregate<double, double>(0, (sd, rt) => sd + Math.Pow(rt - mean, 2)) / (double)(responses.Count - 1));
            }
