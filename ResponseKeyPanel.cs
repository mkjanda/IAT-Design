        ...
        private void SimpleRadio_CheckedChanged(object sender, EventArgs e)
        {
            if (SimpleRadio.Checked)
            {
                LeftSimplePanel = new SimpleResponseKeyPanel(ResponseKeySide.Left, LeftPreviewPane)
                {
                    ...
                    DisplayItemUri = (ModifyResponseKeyCombo.SelectedIndex == -1) ? new DINull().URI : ((ModifyResponseKeyCombo.SelectedItem as CIATKey).LeftValue.Clone() as DIBase).URI,
                };
                RightSimplePanel = new SimpleResponseKeyPanel(ResponseKeySide.Right, RightPreviewPane)
                {
                    ...
                    Size = SimpleResponseKeyPanelSize,
                    DisplayItemUri = (ModifyResponseKeyCombo.SelectedIndex == -1) ? new DINull().URI : ((ModifyResponseKeyCombo.SelectedItem as CIATKey).RightValue.Clone() as DIBase).URI
                };
