...
namespace IATClient
{
    partial class MockItemPanel : UserControl
    {
        ...
        {
            if ((OutlineResponse.Checked) && (KeyedDirDrop.SelectedIndex == -1))
            {
                ParentControl.MainForm.ErrorMsg = Properties.Resources.sNoKeyedDirSelectedForMockItemException;
                return false;
            }
            if (ResponseKeyDrop.SelectedIndex == -1)
            {
                ParentControl.MainForm.ErrorMsg = Properties.Resources.sNoResponseKeySelectedForMockItemException;
                return false; ;
            }
            if (!TextStimulusRadio.Checked && !ImageStimulusRadio.Checked)
            {
                ParentControl.MainForm.ErrorMsg = Properties.Resources.sNoStimulusTypeSelectedForMockItemException;
                throw new Exception();
            }
            if (TextStimulusRadio.Checked)
            {
                if (MockItemTextStimulus.TextValue == String.Empty)
                {
                    ParentControl.MainForm.ErrorMsg = Properties.Resources.sBlankTextStimulusForMockItemException;
                    return false;
                }
            }
            else
            {
                ...
                {
                    ParentControl.MainForm.ErrorMsg = Properties.Resources.sNoImageAssignedToMockItemStimulusException;
                    return false;
                }
            }
