...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    <xsl:element name="CodeFile">
      ...
      <xsl:element name="Classes">
        <xsl:element name="Class">
          ...
          <xsl:attribute name="ClassPrefix" select="'ac'" />
      ...
      xmlDoc.documentElement.appendChild(elem);
      elem = xmlDoc.createElement(&quot;TestSegmentID&quot;);
      elem.appendChild(xmlDoc.createTextNode(this.segmentID));
      xmlDoc.documentElement.appendChild(elem);
        return this.getTextRepresentation(xmlDoc);
            ...
            <xsl:element name="Function">
              ...
              <xsl:variable name="functionBodyElems">
                <xsl:text>
      var strRep = &quot;\n\r&lt;&quot; + elem.localName, ctr;
      if (elem.hasAttributes()) {
      for (ctr = 0; ctr &lt; elem.attributes.length; ctr++) {
  ...
  strRep += &quot;&gt;&quot;;
      ...
      return strRep;
          ...
          strContent += this.recurseElement(xmlDoc.documentElement).trim();
          return strContent;
        </xsl:text>
              </xsl:variable>
              <xsl:element name="FunctionBody">
          ...
          if (respType === &quot;Error&quot;) {
            ...
            while ((elem.nodeType !== 3) &amp;&amp; (elem.nodeType !== 4)) 
              elem = elem.nextSibling;
            if (elem.nodeType === 3) {
              while (elem) {
                elemTextData += elem.data;
                elem = elem.nextSibling;
              }
              elemTextData = elemTextData.substring(&quot;&lt;![CDATA[&quot;.length, elemTextData.lastIndexOf(&quot;]]&gt;&quot;));
            }
            else
              elemTextData = elem.data;
            if (functThis)
              onAjaxSuccess.call(functThis, elemTextData);
            else
              onAjaxSuccess(elemTextData);
            ...
            <xsl:element name="Function">
              ...
              <xsl:variable name="functionBodyElems">
                ...
                xmlhttp.open(&quot;POST&quot;, this.DestURL + &quot;?&quot; + (new Date()).getTime(), true);
      ...
      <xsl:element name="Functions">
        <xsl:element name="Function">
          <xsl:attribute name="FunctionName" select="'timeout'" />
          <xsl:attribute name="FunctionPrefix" select="'acf'" />
          <xsl:element name="Params" />
          <xsl:variable name="functionBodyElems">
            <xsl:text>
