...
namespace IATClient
{
    public class ResultsPanel : Control
    {
        class ServerInterfacePanel : UserControl
        {
            ...
            private Label _SelectedIATLabel = null;
            ...
            public Label SelectedIATLabel
            {
                get
                {
                    return _SelectedIATLabel;
                }
            }
        ...
        public void ShowResultsPanel()
        {
            ...
            catch (Exception ex)
            {
                ...
                IATConfigMainForm.ShowErrorReport("Error initializing results panel", rex);
            }
        }
        ...
        private void IATUploadComplete(IAsyncResult async)
        {
            try
            {
                ...
                Task.Run<bool>(() => IATManager.RetrieveServerReport()).ContinueWith((task) =>
                {
                    ...
                    {
                        CReportableException rex = new CReportableException(ex.Message, ex);
                        IATConfigMainForm.ShowErrorReport("Error retrieving server report", rex);
                    }
                });
            }
            catch (Exception ex)
            {
                CReportableException rex = new CReportableException(ex.Message, ex);
                IATConfigMainForm.ShowErrorReport("Error refreshing IAT list", rex);
            }
            finally
            {
                m_ServerInterfacePanel.EnableControlsAsync(ServerInterfacePanel.EControls.testLabels);
        ...
        private void RetrieveResultsComplete(IAsyncResult async)
        {
            ...
            if ((password = m_ServerInterfacePanel.IATManager.GetIATPasswordFromRegistry(m_ServerInterfacePanel.SelectedIATLabel.Text)) == null)
                password = m_ServerInterfacePanel.PasswordBox.Text;
            Action<String, String> del = new Action<String, String>(IATManager.RetrieveItemSlides);
            del.BeginInvoke(SelectedIAT, password, new AsyncCallback(SlideRetrievalComplete), del);
        }

        private void SlideRetrievalComplete(IAsyncResult async)
        {
            (async.AsyncState as Action<String, String>).EndInvoke(async);
            bool success = IATManager.ItemSlidesDownloaded(SelectedIAT);
            if (success)
            {
                ResultDocument.SetResultData(IATManager.GetResultData(SelectedIAT), IATManager.GetItemSlides(SelectedIAT));
                m_ServerInterfacePanel.EnableControlsAsync(ServerInterfacePanel.EControls.testLabels | ServerInterfacePanel.EControls.exportButton);
                return;
            }
            ...
            {
                Action<String, String> del = new Action<String, String>(IATManager.RetrieveItemSlides);
                String password = IATManager.GetIATPasswordFromRegistry(SelectedIAT);
                if (password == null)
                    password = m_ServerInterfacePanel.PasswordBox.Text;
                del.BeginInvoke(SelectedIAT, password, new AsyncCallback(SlideRetrievalComplete), del);
                return;
