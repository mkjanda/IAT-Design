...
using System.IO;
using IATClient.IATConfigFileNamespace;
...
namespace IATClient
{
    public class CItemSlideContainer : IDisposable
    {
        ...
        public CItemSlideContainer(String iatName, String dataPassword, IATConfigFileNamespace.ConfigFile CF)
        {
            ResizeHalt = new ManualResetEvent(false);
            UpdateHalt = new ManualResetEvent(false);
            _IATName = iatName;
            _Password = dataPassword;
            foreach (var item in CF.EventList.OfType<IATItem>())
                if (item.SlideSaveTask != null)
                    item.SlideSaveTask.Wait();
            var pairings = from item in CF.EventList.OfType<IATItem>() select new { filename = item.ItemSlideFilename, itemNum = (uint)item.ItemNum };
            var filenames = (from pairing in pairings select pairing.filename).Distinct();
        ...
        private bool SetSlideImage(String filename, byte[] imageData)
        {
            ...
            catch (Exception ex)
            {
                ...
                IATConfigMainForm.ShowErrorReport("Error displaying item slides", reportable);
                return false;
            }
        }
