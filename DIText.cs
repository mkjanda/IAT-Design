...
namespace IATClient
{
    ...
    public class TextJustification : Enumeration
    {
        ...
        public static TextJustification FromString(String str)
        {
            ...
            {
                IATConfigMainForm.ShowErrorReport("Error converting text justification valuke", new CReportableException(String.Format("Cannot convert {0} to a text justification value.", str), ex));
                return null;
            }
        }
    }
    ...
    public abstract class DIText : DIGenerated
    {
        ...
        public override Rectangle ItemRect { get; protected set; } = Rectangle.Empty;
        ...
        public override void Save()
        {
            ...
            xDoc.Document.Add(new XElement(BaseType.ToString(),
                ...
                new XElement("LineSpacing", LineSpacing.ToString()), new XElement("Size", new XElement("Width", ItemSize.Width.ToString()),
                new XElement("Height", ItemSize.Height.ToString())), new XElement("Location", new XElement("X", ItemRect.X.ToString()),
                new XElement("Y", ItemRect.Y.ToString())), new XElement("AbsoluteBounds", new XElement("Top", AbsoluteBounds.Top.ToString()),
            ...
            if (this.IImage != null)
                xDoc.Root.Add(new XAttribute("rId", CIAT.SaveFile.GetRelationshipsByType(this.URI, BaseType, this.IImage.BaseType).First().Id));
        ...
        protected override void DoLoad(Uri uri)
        {
            ...
            ItemRect = new Rectangle(Convert.ToInt32(xDoc.Root.Element("Location").Element("X").Value), Convert.ToInt32(xDoc.Root.Element("Location").Element("Y").Value),
                Convert.ToInt32(xDoc.Root.Element("Size").Element("Width").Value), Convert.ToInt32(xDoc.Root.Element("Size").Element("Height").Value));
            ...
            if (xDoc.Root.Attribute("rId") != null)
                SetImage(CIAT.SaveFile.GetRelationship(this, xDoc.Root.Attribute("rId").Value).TargetUri);
        }
        ...
        protected override bool Generate()
        {
            ...
            if (IImage == null)
                ...
                IImage.Image = bmp.Clone() as Bitmap;
            return true;
        }
    ...
    public abstract class DIMultiLineText : DIText
    {
        ...
        protected override bool Generate()
        {
            ...
            Brush phraseBr = new SolidBrush(PhraseFontColor);
            int offset = 0, count = 1;
            while (offset < phrase.Length)
            {
                if (g.MeasureString(phrase.Substring(offset, count).Replace("\r", "*").Replace("\n", "*"), PhraseFont).Width > bSz.Width)
                {
                    while (!Char.IsWhiteSpace(phrase[offset + count - 1]))
                    {
                        count--;
                        if (count == 0)
                        {
                            count = Phrase.Length - offset;
                            break;
                        }
                    }
                    lines.Add(phrase.Substring(offset, count - 1));
                    offset += count;
                    count = 0;
                }
                else if (phrase.Substring(offset).Length > 1 + count)
                {
                    if (phrase.Substring(offset, count - 1).EndsWith("\r\n"))
                    {
                        lines.Add(phrase.Substring(offset, count - 1).Replace("\r", "").Replace("\n", ""));
                        offset += count - 1;
                        count = 0;
                    }
                }
                if (count + offset == phrase.Length)
                {
                    lines.Add(phrase.Substring(offset));
    ...
    public class DIStimulusText : DIText, IStimulus
    {
        ...
        private void OnThumbnailChanged(ImageChangedEvent evt, IImageMedia iMedia, object arg)
        {
        ...
        public override void SetImage(Image img, System.Drawing.Imaging.ImageFormat format)
        {
            ...
            {
                IImage.CreateThumbnail();
                IImage.Thumbnail.Changed += new Action<ImageChangedEvent, IImageMedia, object>(OnThumbnailChanged);
        ...
        public override object Clone()
        {
            ...
            di.SetImage(IImage.Image, IImage.Format);
        ...
        public override void Dispose()
        {
            if (IsDisposed)
                return;
            if (ThumbnailPreviewPanel != null)
                ThumbnailPreviewPanel.SetImage((Bitmap)null);
            base.Dispose();
        }
    }
    public class DIContinueInstructions : DIText
    {
        ...
        public override object Clone()
        {
            ...
            di.SuspendLayout();
            di.Justification = this.Justification;
            di.ItemSize = this.ItemSize;
            di.ItemRect = this.ItemRect;
            di.LineSpacing = this.LineSpacing;
            di.Phrase = this.Phrase;
            di.PhraseFontColor = this.PhraseFontColor;
            di.PhraseFontFamily = this.PhraseFontFamily;
            ...
            di.SetImage(IImage.Image, System.Drawing.Imaging.ImageFormat.Png);
            di.IImage.Changed += (evt, img, arg) => di.OnImageChanged(evt, img, arg);
    ...
    public class DIResponseKeyText : DIText, IResponseKeyDI
    {
        ...
        public void SetKeyOwners(List<IPackagePart> owners)
        {
        ...
        public void ReleaseKeyOwners(List<IPackagePart> owners)
        {
        ...
        protected override void OnImageChanged(Images.ImageChangedEvent evt, Images.IImageMedia img, object arg)
        {
            ...
            if ((evt == Images.ImageChangedEvent.Updated) || (evt == ImageChangedEvent.Initialized))
            {
                ...
                ValidateData?.Invoke();
                foreach (CIATKey key in CIAT.SaveFile.GetRelationshipsByType(this.URI, BaseType, typeof(CIATKey)).Select(pr => CIAT.SaveFile.GetIATKey(pr.TargetUri)))
                    key.ValueChanged();
            }
        }
        ...
        public override object Clone()
        {
            ...
            di.ItemSize = this.ItemSize;
            di.ItemRect = this.ItemRect;
            di.SetKeyOwners(KeyOwners.Select(u => CIAT.SaveFile.GetIATKey(u)).ToList<IPackagePart>());
            di.LineSpacing = this.LineSpacing;
            di.Phrase = this.Phrase;
            di.PhraseFontColor = this.PhraseFontColor;
            di.PhraseFontFamily = this.PhraseFontFamily;
            ...
            di.IImage.Changed += (evt, img, arg) => di.OnImageChanged(evt, img, arg);
            di.AbsoluteBounds = AbsoluteBounds;
            foreach (CIATKey k in KeyOwners.Select(u => CIAT.SaveFile.GetIATKey(u)))
                di.AddKeyOwner(k);
    ...
    public class DIConjunction : DIText, IResponseKeyDI
    {
        ...
        public void SetKeyOwners(List<IPackagePart> newOwners)
        {
        ...
        public override object Clone()
        {
            ...
            di.ValidateData = this.ValidateData;
            di.SetImage(this.IImage.Image, this.IImage.Format);
            di.AbsoluteBounds = AbsoluteBounds;
            di.ResumeLayout(true);
            foreach (CIATKey k in KeyOwners.Select(u => CIAT.SaveFile.GetIATKey(u)))
                di.AddKeyOwner(k);
